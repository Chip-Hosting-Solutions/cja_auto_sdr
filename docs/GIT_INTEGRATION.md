...
| `--include-calculated` | Include calculated metrics inventory in snapshot | - |
| `--include-segments` | Include segments inventory in snapshot | - |

> **Note:** `--include-derived` is not supported with `--git-commit` because derived fields inventory is only available in SDR generation mode. Derived field changes are captured in the standard Metrics/Dimensions diff.
...
└── ProductionAnalytics_dv_12345/
    ...
    ├── calculated-metrics.json   # Optional: when --include-calculated used
    └── segments.json             # Optional: when --include-segments used
```
...
Generated by CJA SDR Generator vX.Y.Z
```
...
## Including Inventory in Snapshots

Git snapshots can optionally include calculated metrics and segments inventory for comprehensive change tracking.

### Basic Inventory Snapshots

```bash
# Include calculated metrics inventory
cja_auto_sdr dv_12345 --git-commit --include-calculated

# Include segments inventory
cja_auto_sdr dv_12345 --git-commit --include-segments

# Include both inventories
cja_auto_sdr dv_12345 --git-commit --include-calculated --include-segments

# With custom message
cja_auto_sdr dv_12345 --git-commit --include-calculated --include-segments \
  --git-message "Weekly inventory snapshot"
```

### Extended Directory Structure

When inventory flags are used, additional files are created:
...
└── ProductionAnalytics_dv_12345/
    ...
    ├── calculated-metrics.json   # When --include-calculated used
    └── segments.json             # When --include-segments used
```
...
# See calculated metrics changes
git diff HEAD~1 -- */calculated-metrics.json

# See segment changes
git diff HEAD~1 -- */segments.json

# See all inventory changes across data views
git diff HEAD~1 -- */*.json
```

### Derived Fields Note

The `--include-derived` flag is **not supported** with `--git-commit` because:
- Derived fields inventory is only available in SDR generation mode
- Derived field changes are captured in the standard Metrics/Dimensions diff
- The metrics/dimensions data stored in `metrics.json` and `dimensions.json` already contains derived field information

```bash
# This works: SDR generation with derived fields inventory
cja_auto_sdr dv_12345 --include-derived

# This errors: not supported with git snapshots
cja_auto_sdr dv_12345 --git-commit --include-derived
# ERROR: --include-derived cannot be used with --git-commit
```
